...
public class PlayerAIAggressive : Player
{
    ...
    public void AttackSmaller(Town originatingTown_)
    {
        ...
        foreach (Player player in players)
        {
            ...
            List<Town> towns = GetTownsBelowMaxPopulation(originatingTown_.GetAttackerPopulation(), player);

            if (towns.Count > 0)
            {
                originatingTown_.SendAttack(towns[UnityEngine.Random.Range(0, towns.Count)]);
    ...
    public List<Town> GetTownsBelowMaxPopulation(int maxPopulation_, Player player_)
    {
        List<Town> towns = new List<Town>();
        List<Town> playersTowns = GameInterface.instance.GetPlayerTowns(player_);
        foreach(Town town in playersTowns)
        {
            if(town.GetMaxPopulation() < maxPopulation_)
            {
                towns.Add(town);
            }
        }
